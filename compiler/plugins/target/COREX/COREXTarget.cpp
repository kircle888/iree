...
static FailureOr<std::string> findPtxasCompiler(const COREXOptions &options,
                                                std::string *message) {
...
public:
  ...
  std::string name() const override { return "corex"; }
  ...
  LogicalResult serializeExecutable(const SerializationOptions &serOptions,
    ...
    if (variantOp.isExternal()) {
      ...
      for (auto exportOp : variantOp.getExportOps()) {
        ...
        printf("Exported Op Name %s\n", exportOp.getSymName().str().c_str());
      }
      ...
      for (auto [exportOp, workgroupSize] :
        ...
        entryPointNames.emplace_back(sanitizeSymbolName(exportOp.getName()));
      }
#if 0
      auto llvmModule =
      ...
      // final FlatBuffer.
      llvm::raw_string_ostream ss(ptxImage);
      if (failed(emitcuda::translateToCuda(innerModuleOp, ss))) {
        return failure();
      }
      // ss << *innerModuleOp;
      ss.flush();
...
private:
  ...
  getExecutableTarget(MLIRContext *context) const {
    ...
    return IREE::HAL::ExecutableTargetAttr::get(
        context, b.getStringAttr("corex"), b.getStringAttr("corex-llvm-ir"),
        configAttr);
  }
...
extern "C" bool iree_register_compiler_plugin_hal_target_corex(
