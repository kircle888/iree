...
struct COREXOptions {
  ...
  bool clUseClang = false;
  std::string clUseClangFrom;
  std::string clUseClangParams;

  void bindOptions(OptionsBinder &binder) {
    static llvm::cl::OptionCategory category("COREX HAL Target");

    // binder.opt<std::string>(
    //     "iree-corex-target", clTarget, llvm::cl::cat(category),
    //     llvm::cl::desc(
    //         // clang-format off
    //         "COREX target as expected by LLVM NVPTX backend; e.g., "
    ...
    //         "https://iree.dev/guides/deployment-configurations/gpu-corex "
    //         "for more details."
    //         // clang-format on
    //         ));

    // binder.opt<std::string>(
    //     "iree-corex-target-features", clTargetFeatures,
    //     llvm::cl::cat(category), llvm::cl::desc(
    //         "COREX target features as expected by LLVM NVPTX backend; e.g. "
    //         "use '+ptxNN' to set PTX version to NN."));

    binder.opt<bool>(
        "iree-corex-use-clang", clUseClang, llvm::cl::cat(category),
        llvm::cl::desc(
            "Whether to use the clang tool to assemble the generated PTX "
            "code and put the generated CUBIN binary file into the executable. "
            "If not set, directly embeds the PTX into the executable. "
            "To specify the exact clang tool path, use "
            "'--iree-corex-use-clang-from'. To pass "
            "additional parameters to clang, use "
            "'--iree-corex-use-clang-params', e.g. "
            "'--iree-corex-use-clang-params=-v'"));

    binder.opt<std::string>(
        "iree-corex-use-clang-from", clUseClangFrom, llvm::cl::cat(category),
        llvm::cl::desc("Uses the clang tool from the given path. Requires "
                       "'--iree-corex-use-clang' to be true."));

    binder.opt<std::string>(
        "iree-corex-use-clang-params", clUseClangParams,
        llvm::cl::cat(category),
        llvm::cl::desc("Passes the given additional parameters to clang. "
                       "Requires '--iree-corex-use-clang' to be true."));
  }

  LogicalResult verify(mlir::Builder &builder) const {
    // if (GPU::normalizeCOREXTarget(clTarget).empty()) {
    //   return emitError(builder.getUnknownLoc(), "Unknown COREX target '")
    //          << clTarget << "'";
    // }
    return success();
  }
...
static constexpr char kCompilerName[] = "clang++";

/// Attempts to find clang compiler
static FailureOr<std::string> findClangCompiler(const COREXOptions &options,
                                                std::string *message) {
  std::string clangCompiler;
  if (!options.clUseClangFrom.empty())
    clangCompiler = options.clUseClangFrom;
  if (llvm::sys::fs::exists(clangCompiler))
    return clangCompiler;

  clangCompiler = findTool(kCompilerName);
  if (llvm::sys::fs::exists(clangCompiler))
    return clangCompiler;

  *message = std::string(
      "Could not find clang compiler. Try passing it explicitly with "
      "--iree-corex-use-clang-from=<path> flag");
  return failure();
}

/// Compiles the given generated PTX code with the given clang compiler.
static FailureOr<std::string> compileWithClang(StringRef clangCompiler,
                                               StringRef smCapability,
                                               StringRef clangParams,
                                               StringRef ptxSource,
                                               std::string *message) {
  ...
  llvm::sys::fs::createTemporaryFile("clang-stdin", "", stdinFile);
  llvm::sys::fs::createTemporaryFile("clang-stdout", "", stdoutFile);
  llvm::sys::fs::createTemporaryFile("clang-stderr", "", stderrFile);
  ...
  // Step 2. Write the generated PTX into a file, so we can pass it to clang
  // compiler
  std::error_code ec;
  ...
  // Step 3. Build the clang command line
  std::vector<StringRef> ArgVector{
      StringRef(kCompilerName), StringRef("-arch"), smCapability,
  ...
  Tokenize(clangParams, stringSaver, rawArgs, /*MarkEOLs=*/false);
  ...
  // Step 4. Invoke clang
  if (llvm::sys::ExecuteAndWait(unescapeCommandLineComponent(clangCompiler),
                                ...
                                /*ErrMsg=*/message)) {
    if (message->empty()) {
      *message = std::string("Invoking clang is failed, see the file: ") +
  ...
  // Step 5. The output of clang if verbose flag is set. This is useful
  // because it shows local memory usage, register usage, and etc.
  if (clangParams.find("-v") != StringRef::npos) {
...
// Attempt compiling the PtxImage with clang compiler. If the compilation fails
// for some reason return and pack the generated PtxImage code in the
// executable, let the runtime compile.
static std::string produceGpuImage(const COREXOptions &options,
                                   StringRef targetArch,
                                   std::string &ptxImage) {
  if (!options.clUseClang)
    return ptxImage;

  std::string message;
  FailureOr<std::string> clangCompiler = findClangCompiler(options, &message);

  if (succeeded(clangCompiler)) {
    FailureOr<std::string> maybeCubinImage =
        compileWithClang(clangCompiler.value(), targetArch,
                         options.clUseClangParams, ptxImage, &message);
    if (succeeded(maybeCubinImage))
      return maybeCubinImage.value();
  }

  llvm::WithColor::warning()
      << "Compilation with `clang` failed, the generated ptx will be "
...
class COREXTargetDevice final : public TargetDevice {
public:
  COREXTargetDevice(const COREXOptions &options) : options(options) {}
  ...
  getDefaultDeviceTarget(MLIRContext *context,
    ...
    targetRegistry.getTargetBackend("corex")->getDefaultExecutableTargets(
        context, "corex", executableConfigAttr, executableTargetAttrs);

    return IREE::HAL::DeviceTargetAttr::get(context, b.getStringAttr("corex"),
                                            deviceConfigAttr,
                                            executableTargetAttrs);
  }

private:
  const COREXOptions &options;
};

class COREXTargetBackend final : public TargetBackend {
public:
  COREXTargetBackend(const COREXOptions &options) : options(options) {}

  std::string getLegacyDefaultDeviceID() const override { return "corex"; }
  ...
  getExecutableTarget(MLIRContext *context) const {
    ...
    addConfig("iree.gpu.target",
              GPU::getCUDATargetDetails("sm_70", "",
                                        context)); // use V100 as corex target

    return b.getAttr<IREE::HAL::ExecutableTargetAttr>(
        b.getStringAttr("corex"), b.getStringAttr("corex-nvptx-fb"),
      ...
      llvm::errs() << "Start Translation" << "\n";
        ...
        if (!serOptions.dumpBinariesPath.empty()) {
          dumpDataToPath(serOptions.dumpBinariesPath, serOptions.dumpBaseName,
                         variantOp.getName(), ".cu", targetPTX);
        }
      }
      ...
      for (auto exportOp : exportOps) {
        ...
        if (!ordinalAttr) {
          return mlir::emitError(exportOp.getLoc())
                 << "could not compile rocm binary: export op is missing "
                    "ordinal";
        }
        ...
        iree_hal_cuda_ExportDef_module_ordinal_add(builder,
                                                   0); // always 0 today
  ...
  namespace {
  struct COREXSession
      : public PluginSession<COREXSession, COREXOptions,
                             PluginActivationPolicy::DefaultActivated> {
    void populateHALTargetDevices(IREE::HAL::TargetDeviceList &targets) {
      // #hal.device.target<"corex", ...
      targets.add("corex", [&]() {
        return std::make_shared<COREXTargetDevice>(options);
      });
    }
    void populateHALTargetBackends(IREE::HAL::TargetBackendList &targets) {
      // #hal.executable.target<"corex", ...
      targets.add("corex", [&]() {
        LLVMInitializeNVPTXTarget();
        LLVMInitializeNVPTXTargetMC();
        LLVMInitializeNVPTXTargetInfo();
        LLVMInitializeNVPTXAsmPrinter();
        return std::make_shared<COREXTargetBackend>(options);
      });
    }
  };
...
extern "C" bool
iree_register_compiler_plugin_hal_target_corex(
    mlir::iree_compiler::PluginRegistrar *registrar) {
  registrar->registerPlugin<mlir::iree_compiler::IREE::HAL::COREXSession>(
      "hal_target_corex");
  return true;
}

IREE_DEFINE_COMPILER_OPTION_FLAGS(mlir::iree_compiler::IREE::HAL::COREXOptions);
