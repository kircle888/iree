...
static void printInt(CudaEmitter &emitter, const APInt &val, Type iType) {
  ...
  if (val.getBitWidth() == 1) {
    ...
    if (auto idxType = dyn_cast<IndexType>(iType)) {
      postfix = "lu";
    } else if (auto itType = dyn_cast<IntegerType>(iType)) {
...
static LogicalResult printConstantOp(CudaEmitter &emitter, Operation *operation,
  ...
  if (auto vType = dyn_cast<VectorType>(operation->getResult(0).getType())) {
    ...
    if (auto dense = dyn_cast<DenseFPElementsAttr>(value)) {
      ...
      if (dyn_cast<Float32Type>(fType)) {
        typeNoter = "(float)";
      } else if (dyn_cast<Float64Type>(fType)) {
        typeNoter = "(double)";
      } else if (dyn_cast<Float16Type>(fType)) {
        typeNoter = "(half)";
      } else if (dyn_cast<BFloat16Type>(fType)) {
        typeNoter = "(bfloat16)";
      } else {
...
bool CudaEmitter::shouldMapToUnsigned(Type val) {
  if (auto iType = dyn_cast<IntegerType>(val)) {
    return shouldMapToUnsigned(iType.getSignedness());
  }
  if (auto iType = dyn_cast<IndexType>(val)) {
    return true;
  }
  if (auto vType = dyn_cast<VectorType>(val)) {
...
static LogicalResult printOperation(CudaEmitter &emitter, arith::CmpIOp op) {
  ...
  auto checkType = [&](Type t) -> LogicalResult {
    if (t.isIntOrIndex())
      return success();
    if (dyn_cast<VectorType>(t))
      return success();
    if (dyn_cast<TensorType>(t))
...
static bool isBoolOperation(Operation &op) {
  IntegerType iType;
  if (auto lType = dyn_cast<VectorType>(op.getOperand(0).getType())) {
    iType = dyn_cast<IntegerType>(lType.getElementType());
  } else {
    iType = dyn_cast<IntegerType>(op.getOperand(0).getType());
  }
  if (!iType || (iType.getWidth() != 1))
    return false;
  if (auto lType = dyn_cast<VectorType>(op.getOperand(1).getType())) {
    iType = dyn_cast<IntegerType>(lType.getElementType());
  } else {
    iType = dyn_cast<IntegerType>(op.getOperand(1).getType());
  }
  if (!iType || (iType.getWidth() != 1))
    return false;
  return true;
...
static LogicalResult printVecCastOperation(CudaEmitter &emitter,
  ...
  if (auto vType = dyn_cast<VectorType>(oType)) {
...
static LogicalResult printOperation(CudaEmitter &emitter, arith::BitcastOp op) {
  ...
  if (isa<VectorType>(outType)) {
    os << "bitcast<";
    if (failed(emitter.emitType(
            op->getLoc(), dyn_cast<VectorType>(outType).getElementType()))) {
      return failure();
    }
    os << ">";
    os << "(" << emitter.getOrCreateName(op.getIn()) << ")";
...
static LogicalResult printMemrefDeclare(CudaEmitter &emitter, Location loc,
  ...
  auto mType = dyn_cast<MemRefType>(value.getType());
  if (!mType) {
...
static LogicalResult printOperation(CudaEmitter &emitter,
  ...
  Operation *operation = op.getOperation();
  auto vecType = dyn_cast<VectorType>(op.getResult().getType());
  if (failed(emitter.emitAssignPrefix(*operation))) {
    return failure();
  }
  ...
  auto dim = vecType.getShape().size() -
             dyn_cast<VectorType>(op.getSource().getType()).getShape().size();
...
static LogicalResult printOperation(CudaEmitter &emitter,
  ...
  if (auto sType = dyn_cast<LLVM::LLVMStructType>(inputType)) {
    if (!isMemrefStruct((sType))) {
      return op->emitOpError("Unknown UnrealizedConversionCastOp, "
                             "input struct is not memref struct");
    }
    auto mType = dyn_cast<MemRefType>(op->getResult(0).getType());
    if (!mType) {
  ...
  if (auto iType = dyn_cast<IndexType>(inputType)) {
    if (failed(emitter.emitAssignPrefix(*operation))) {
      return failure();
    }
    os << emitter.getOrCreateName(op->getOperand(0));
